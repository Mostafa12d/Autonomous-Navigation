...
data = readmatrix('angular_velocity.csv');
omega_x = data(:, 2);
omega_y = data(:, 3);
omega_z = data(:, 4);
Fs = 40;
...
[avarFromFunc, tauFromFunc] = allanvar(omega_x, m, Fs);
...
xlabel('\tau')
ylabel('\sigma(\tau)')
legend('$\sigma (rad/s)$ y-axis ', '$\sigma_N ((rad/s)/\sqrt{Hz})$ Angle Random Walk', ...
    '$\sigma_K ((rad/s)\sqrt{Hz})$ Rate Random Walk', '$\sigma_B (rad/s)$ Bias Instability', 'Interpreter', 'latex')
